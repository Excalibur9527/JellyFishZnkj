...
- 分镜帧最终图片提示词会按参考图类型追加统一的“参考图使用规则”：
  - `character` 参考图只负责人物身份、脸型、五官、发型、气质、体态与身份一致性
...
- OpenAI 兼容图片编辑接口当前使用 `multipart/form-data` 上传参考图：
  ...
  - 多张本地参考图会作为重复的 `image` 文件字段上传，而不是 `image[0] / image[1]`
  ...
  - 带参考图的 GPT Image 编辑请求默认追加 `input_fidelity=high`，让模型更努力保留输入图特征，尤其是角色脸部特征
...
- 分镜帧若包含人物，最终图片提示词会追加视频参考图安全风格约束：要求保留清晰可辨的原创虚构人脸与完整自然五官，呈现为高质量、轻微风格化的影视概念参考图，同时避免无脸、遮脸、背影替代、面部模糊、真实摄影照片质感以及真实个人/明星/版权角色相似
  - 若存在 `character` 参考图，系统会额外追加人物身份锁定：角色图是人物身份与脸部特征的唯一来源，必须保持脸型轮廓、五官比例、眼型、鼻型、嘴型、下巴轮廓、发际线、发型、神态气质和年龄感；服装、场景、道具参考图中的人脸不得作为身份来源
