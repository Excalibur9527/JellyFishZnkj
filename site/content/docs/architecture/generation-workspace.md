...
当前音频链路先以“对白配音”为最小闭环，不再假设视频供应商会自动产出声音：

- `FileType` 当前支持 `audio`，上传 / 下载 / 本地存储 MIME 推断都会识别常见音频后缀。
- `FileUsageKind` 当前支持 `generated_audio`，用于标记由系统生成的镜头音频文件。
- `ShotAudioClip` 记录镜头内音频片段：
  - `shot_id`
  - `file_id`
  - `dialogue_line_id`
  - `clip_type`
  ...
  - `track`
  - `provider / voice`
- 工作室当前会按选中镜头读取 `/api/v1/studio/audio/shots/{shot_id}/clips`，并在“对白状态”中展示可试听音频。
...
- `/api/v1/studio/audio/shots/{shot_id}/clips` 当前支持把已上传音频绑定到镜头，可作为 BGM/音效/临时外部配音片段使用。
- `/api/v1/studio/audio/shots/{shot_id}/mux-video` 当前作为“有声视频合成”入口：
